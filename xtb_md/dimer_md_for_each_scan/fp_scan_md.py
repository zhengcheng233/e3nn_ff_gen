    ...
    for idx, ii in enumerate(task_set):
        with open(task_name+'_'+str(idx)+'.slurm','w') as fp:
            ...
            for jj in ii:
                ...
                conf_name = jj.split('/')[1]
                conf_path = os.path.abspath(os.path.join('./',conf_name))
                tmp_path = '/tmp/scratch/'+str(user_name)+'/'+task_name+'.$SLURM_JOB_ID/'+jj[2:-len(file_name)]
                dir_path = tmp_path 
                fp.write('mkdir -p ' + dir_path + '\n')
                fp.write('cp '+abs_path+'input.gjf '+ tmp_path+'\n')
                fp.write('sleep 0.5'+'\n')
                fp.write('cp '+_cwd+'/__init__.py '+ tmp_path+'\n') 
                fp.write('sleep 0.5'+'\n')
                fp.write('cp '+_cwd+'/topo_dimer.py '+ tmp_path+'\n') 
                fp.write('sleep 0.5'+'\n')
                fp.write('cp '+_cwd+'/relax_md.py '+ tmp_path+'\n') 
                fp.write('sleep 0.5'+'\n')
                fp.write('cp '+_cwd+'/conf.bondlength '+ tmp_path+'\n') 
                fp.write('sleep 0.5'+'\n')
                fp.write('cp '+conf_path+'/dimer_num.txt ' + tmp_path+'\n')
                fp.write('sleep 0.5'+'\n')
                fp.write('cp '+conf_path+'/dimer_topo.txt ' + tmp_path+'\n')
                fp.write('sleep 0.5'+'\n')                

                file_name = os.path.basename(jj)
                fp.write('cd /tmp/scratch/'+str(user_name)+'/'+task_name+'.$SLURM_JOB_ID'+'\n')
                fp.write('cd '+str(jj[:-len(file_name)])+'\n')
                fp.write('python relax_md.py > tmp.log'+'\n')
                file_name = os.path.basename(jj) 
                abs_path = os.path.abspath(jj)[:-len(file_name)]
                tmp_path = '/tmp/scratch/'+str(user_name)+'/'+task_name+'.$SLURM_JOB_ID/'+jj[2:-len(file_name)]
                fp.write('cp -r '+tmp_path+'coord.npy ' + abs_path+'\n')
                fp.write('sleep 0.5'+'\n')
...
if __name__ == '__main__':
    ...
    files = glob('./conf.000_000/gjfs/*/*/input.gjf')
    print(len(files))
    files = files[0:1000]
