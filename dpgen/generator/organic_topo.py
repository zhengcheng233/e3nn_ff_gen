...
def _crd2frag(symbols, crds, q_net, pbc=False, cell=None):
...
if __name__ == '__main__':
        ...
        with open(dir0) as fp:
            for line in fp:
                ...
                if len(line) == 2 and line[-1] == '1':
                    q_net = int(line[0])
                if len(line) == 4:
                    coord.append([float(u) for u in line[1:]])
                    symbol.append(line[0])
        return coord, symbol, q_net
    ...
    for dir0 in dirs:
        ...
        os.chdir(dir_name)
        coord, symbol, qnet = conf_read(f_name)
        bond, proton = _crd2frag(symbol, coord, qnet)
